export interface CreateBookingRequest { fullName: string; email: string; phone: string; membershipId: string; classId?: string; scheduleId?: string; bookingDate?: string }

export interface Schedule { id: string; className: string; tutorName?: string | null; code?: string | null; dayOfWeek: string; startTime: string; endTime: string; isActive: boolean }
export interface Membership { id?: string; name?: string; type?: string; price?: number; priceNGN?: number; duration?: number; period?: string; classLimit?: number | null; status?: string; startDate?: string | null; endDate?: string | null; expiryDate?: string | null; autoRenew?: boolean; features?: string[] }
export interface HealthSafetyForm { id?: string; submittedAt?: string | null; createdAt?: string | null }
export interface Booking { id: string; fullName: string; email: string; phone: string; classId: string | null; scheduleId: string | null; bookingDate: string | null; amount: number; paymentReference: string; paymentStatus: 'PENDING' | 'PAID' | 'FAILED'; bookingStatus: 'PENDING' | 'CONFIRMED' | 'CANCELLED'; membershipId: string; userId: string | null; createdAt: string; updatedAt: string; membership?: Membership | null; schedule?: Schedule | null; healthSafetyForm?: HealthSafetyForm | null; preferredStartDate?: string | null; availableDays?: string[]; preferredTimes?: string[] }
export interface CreateBookingResponse { success: boolean; message?: string; data: { booking: Booking; authorizationUrl: string } }
...
export async function updateBookingPart(bookingId: string, part: 'class' | 'schedule' | 'start-date', body: Record<string, string>) { return request(`/api/bookings/${bookingId}/${part}`, { method: 'PATCH', body: JSON.stringify(body) }) }
...
export async function createBooking(data: CreateBookingRequest): Promise<CreateBookingResponse> { return request<CreateBookingResponse>('/api/bookings', { method: 'POST', body: JSON.stringify(data) }) }
