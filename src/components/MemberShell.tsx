...
const isActive = (href: string) => {
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      <aside
        className={`fixed inset-y-0 left-0 z-40 w-72 border-r border-border bg-card p-6 transition-transform lg:translate-x-0 ${
          open ? 'translate-x-0' : '-translate-x-full'
        }`}
      >
        <div className="flex items-center justify-between">
          <Link href="/" aria-label="Sculpt LAB home">
            <Image
              src="/logo.png"
              alt="Sculpt LAB Logo"
              width={76}
        ...
        <button
          onClick={signOut}
          className="absolute inset-x-6 bottom-6 flex items-center gap-3 border-t border-border px-3 pt-5 text-sm text-muted-foreground hover:text-primary"
        >
      ...
      <div className="lg:pl-72">
        <header className="sticky top-0 z-30 flex h-20 items-center border-b border-border bg-background/95 px-4 backdrop-blur">
          <button
