...
import { getMyBookings, fetchMemberships, getAdminDashboard, type Booking, type Membership, type AdminDashboardData } from '@/lib/api/booking'
...
export async function loadAdminDashboard(): Promise<AdminResource<AdminDashboardData>> { try { const data = await getAdminDashboard(); return { state: 'ready', data } } catch (error) { return { state: 'error', message: error instanceof Error ? error.message : 'Unable to load the admin dashboard.' } } }
